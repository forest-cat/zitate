...
logger = logging.getLogger('app.cogs.commands')
...
class Commands(commands.Cog):
    ...
    async def zitat(self, ctx: discord.ApplicationContext,
                    discord_benutzer: Option(discord.Member, "Der Discord-Benutzer, der zitiert werden soll",
                                             required=True),
        ...
        if len(zitat) > 1500:
            await ctx.respond("Leider ist das Zitat zu lang (maximal `1500` Zeichen), bitte kürze es ein wenig",
                              ephemeral=True)
            return
        if len(kontext) > 400:
            await ctx.respond("Leider ist der Kontext zu lang (maximal `400` Zeichen), bitte kürze ihn ein wenig",
                              ephemeral=True)
            return
        ...
        await ctx.respond(
            f'Der Benutzer {discord_benutzer.mention} wurde mit folgendem Zitat zitiert: `{zitat}` von {ctx.author.mention}!',
            ephemeral=True)
        ...
        cursor.execute("""INSERT INTO Zitate (Zitat_User_Name,
                                              Writer_User_Name,
                                              Zitat_User_ID,
                                              Writer_User_ID,
                                              Zitat,
    ...
    # The /custom_zitat command
    @slash_command(guild_ids=settings.guilds)
    @commands.cooldown(1, settings.quote_cooldown, commands.BucketType.user)
    @commands.has_role(settings.quote_permission_role)
    async def custom_zitat(self, ctx: discord.ApplicationContext,
        ...
        message = await channel.send(embed=zitat_embed)
        await ctx.respond(
            f'**@{benutzer.capitalize()}** wurde mit folgendem Zitat zitiert: `{zitat}` von {ctx.author.mention}!',
            ephemeral=True)
        await message.add_reaction(f"<:{settings.upvote_emoji_name}:{settings.upvote_emoji_id}>")
        await message.add_reaction(f"<:{settings.downvote_emoji_name}:{settings.downvote_emoji_id}>")
        conn = sqlite3.connect(settings.db_filename)
    ...
    # The App Integration for quoting discord messages
    @message_command(guild_ids=settings.guilds, name="Zitieren")
    @commands.cooldown(1, settings.quote_cooldown, commands.BucketType.user)
    @commands.has_role(settings.quote_permission_role)
    async def app_zitat(self, ctx: discord.ApplicationContext, message):
    ...
    async def send_info_text(self, ctx):
        ...
        info_embed = discord.Embed(
            ...
            description=f"""
                ...
                \n_ _
                """)
        info_embed.set_footer(icon_url=self.bot.user.avatar.url,
                              text=f"{self.bot.user.display_name} am {datetime.datetime.now().strftime('%d.%m.%Y um %H:%M')}")
